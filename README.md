...
以下の設定はすべてGitHub/DNS管理画面で手動で行う。コードにCNAMEファイルは作成しない。

1. **DNSの設定**
   `assets` のCNAMEレコードを `mtoya22.github.io` に向ける

2. **GitHubアカウントのドメイン検証**
   GitHub → Settings → Pages → Verified domains で `fire-lifeplan-lab.com` を検証する

3. **リポジトリのPages設定**
   Settings → Pages → Custom domain に `assets.fire-lifeplan-lab.com` を入力
   「Enforce HTTPS」を有効化する

4. **Vite base**
   `vite.config.ts` の `base: '/'` のまま変更不要（サブドメイン運用のため）
